 ...
 * `package.json`, which is what the release workflow checks against the tag.
 *
 ...
 * (`LOGNOTIFIER_BUILD_COUNTER=1`). A plain `npm run build`, and with it the
...
export function wantsBuildNumber() {
  const flag = process.env.LOGNOTIFIER_BUILD_COUNTER;
